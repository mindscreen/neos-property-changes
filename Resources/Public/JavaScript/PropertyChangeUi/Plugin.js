...
var _neosUiBackendConnector = __webpack_require__(12);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var _marked = /*#__PURE__*/regeneratorRuntime.mark(watchGuestFrameLoaded);
...
var propertyChangeRegistry = {};
var nodeTypesRegistry = null;
var translationRegistry = null;

var createHint = function createHint(container, propertyName, propertyLabel) {
...
var createHints = function createHints(container, nodeType, properties) {
    container.innerHTML = '';
...
var registerPropertyChangeSlots = function registerPropertyChangeSlots() {
    var doc = getGuestFrameDocument();
    propertyChangeRegistry = {};
    [].concat(_toConsumableArray(doc.querySelectorAll('[data-__property-change-observe]'))).forEach(function (element) {
        var observedProperties = JSON.parse(element.getAttribute('data-__property-change-observe')).filter(function (s) {
            return s.trim() !== '';
        });
        var changedProperties = JSON.parse(element.getAttribute('data-__property-change-changedproperties')).filter(function (s) {
            return s.trim() !== '';
        });
        var contextPath = element.getAttribute('data-__property-change-contextpath');
        var nodeType = element.getAttribute('data-__property-change-nodetype');
        var elementData = {
            element: element,
            notify: element.hasAttribute('data-__property-change-notify') ? JSON.parse(element.getAttribute('data-__property-change-notify')) : {},
        ...
        propertyChangeRegistry[contextPath].observedProperties = (0, _lodash2.default)(propertyChangeRegistry[contextPath].observedProperties, observedProperties);
        createHints(element, nodeType, changedProperties);
    });
};

function watchGuestFrameLoaded() {
    return regeneratorRuntime.wrap(function watchGuestFrameLoaded$(_context) {
        while (1) {
            switch (_context.prev = _context.next) {
                case 0:
                    _context.next = 2;
                    return (0, _effects.takeLatest)(_neosUiReduxStore.actionTypes.UI.ContentCanvas.STOP_LOADING, function () {
...
var getPropertyLabel = function getPropertyLabel(nodeType, propertyName) {
    var nodeTypeDefinition = nodeTypesRegistry.get(nodeType);
    if (!nodeTypeDefinition) {
        return propertyName;
    }
    var label = (0, _plowJs.$get)('properties.' + propertyName + '.ui.label', nodeTypeDefinition);
    if (!label) {
        return propertyName;
    }
...
var handlePropertyChanges = function handlePropertyChanges(contextPath, nodeData) {
    var _propertyChangeRegist = propertyChangeRegistry[contextPath],
        elements = _propertyChangeRegist.elements,
        observedProperties = _propertyChangeRegist.observedProperties;

    var changedProperties = [];
    var changedPropertiesStateProperty = 'propertyChangeState';
    if (changedPropertiesStateProperty in nodeData.properties) {
        var changedPropertiesState = JSON.parse(nodeData.properties[changedPropertiesStateProperty] || '[]');
        var _iteratorNormalCompletion = true;
        var _didIteratorError = false;
        var _iteratorError = undefined;

        try {
            for (var _iterator = observedProperties[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                var propertyName = _step.value;

                if (!(propertyName in changedPropertiesState) || nodeData.properties[propertyName] !== changedPropertiesState[propertyName]) {
        ...
        try {
            for (var _iterator2 = observedProperties[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                var _propertyName = _step2.value;

                if (_propertyName in nodeData.properties) {
                    changedProperties.push(_propertyName);
                }
            }
        } catch (err) {
    ...
    try {
        for (var _iterator3 = elements[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
            ...
            try {
                for (var _iterator4 = changedProperties[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                    var _propertyName2 = _step4.value;

                    if (_propertyName2 in elementData.notify) {
                        notifyProperties = (0, _lodash2.default)(notifyProperties, elementData.notify[_propertyName2]);
                    }
                }
            } catch (err) {
...
(0, _neosUiExtensibility2.default)('Mindscreen.PropertyChanges:PropertyChangeUi', {}, function (globalRegistry) {
    /** @var {SynchronousRegistry} */
    var sagaRegistry = globalRegistry.get('sagas');
    ...
    /** @var {SynchronousRegistry} */
    var serverFeedbackHandlersRegistry = globalRegistry.get('serverFeedbackHandlers');
    var updateNodeInfoHandlerKey = 'Neos.Neos.Ui:UpdateNodeInfo/Main';
    var original = serverFeedbackHandlersRegistry.get(updateNodeInfoHandlerKey);
    var decorated = function decorated(feedback, store) {
        var changesByContextPath = feedback.byContextPath;
        var _iteratorNormalCompletion5 = true;
        var _didIteratorError5 = false;
        var _iteratorError5 = undefined;

        try {
            for (var _iterator5 = Object.keys(changesByContextPath)[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                var contextPath = _step5.value;

                if (!(contextPath in propertyChangeRegistry)) {
                    continue;
                }
                var nodeData = changesByContextPath[contextPath];
